...
- **Antipattern-file naming uniformity (closed 2026-05-09).** Previously seven skills (stack-ready, repo-ready, production-ready, deploy-ready, observe-ready, launch-ready, harden-ready) lacked a dedicated `references/<skill>-antipatterns.md` file. The coordinated antipattern-catalog sweep landed all seven at once: each ships a named-failure-mode catalog with grep tests, severity ladder, and per-skill guards. ~1,100 lines of content total. All 11 skills now follow the `<skill>-antipatterns.md` naming convention.
- **Hub `CHANGELOG.md` (added 2026-05-09).** Earlier this skill maintained that the hub did not need a CHANGELOG because it had no version stream and `git log` served the same purpose. That rationalization was retired. The hub now ships `CHANGELOG.md` (unversioned by design; entries are dated, not numbered) at the root. A curated narrative is materially more useful for readers than raw commit log; the trade-off (one more file to keep up to date) is small.
