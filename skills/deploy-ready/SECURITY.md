...
2. **Email** to `hprincivil@gmail.com` with subject line `SECURITY: deploy-ready` and a clear description of the issue, reproduction steps, and any proposed mitigation.
...
The skill follows semantic versioning. Security fixes land on the latest minor release; older minors do not receive backports. Users are encouraged to track the latest release via the `aihxp/deploy-ready/releases` feed.
